...
BloomFilter::BloomFilter(int bits) : bits_key(bits) {
  hash_size = static_cast<size_t>(bits * 0.69); // 0.69 =~ ln(2),limit hash size
  if (hash_size < 1)
    hash_size = 1;
  if (hash_size > 30)
    hash_size = 30;
}
void BloomFilter::CreateFiler(const string *key, std::string *filter) const {
...
bool BloomFilter::KeyMayMatch(const string &key, const string &filter) const {
